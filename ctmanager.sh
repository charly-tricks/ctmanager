...
instalar_socks() {
    ...
    # Descargar proxy.py desde GitHub
    wget -q -O "$SOCKS_DIR/proxy.py" "https://raw.githubusercontent.com/charly-tricks/ctmanager/main/proxy.py"
    if [ $? -ne 0 ] || [ ! -s "$SOCKS_DIR/proxy.py" ]; then
        echo -e "${RED}  [!] Error descargando proxy.py, creando localmente...${NC}"
...
def handle_client(cs, dh, dp, pc):
    p = {"200": b"HTTP/1.1 200 OK
Content-Length: 0
...
"}
    try:
        data = cs.recv(4096)
        if not data: cs.close(); return
        cs.sendall(p.get(pc, p["200"]))
        d = socket.socket(); d.connect((dh, int(dp)))
        threading.Thread(target=forward, args=(cs, d), daemon=True).start()
        threading.Thread(target=forward, args=(d, cs), daemon=True).start()
    except:
        try: cs.close()
        except: pass

def start(lp, dh, dp, pc):
    s = socket.socket(); s.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    s.bind(("0.0.0.0", lp)); s.listen(200)
    print(f"[SOCKS] {lp} -> {dh}:{dp} | {pc}")
    while True:
        c, _ = s.accept()
        threading.Thread(target=handle_client, args=(c, dh, dp, pc), daemon=True).start()
