...
instalar_websocket() {
    ...
    read -p "  Host destino TCP (ej: 127.0.0.1): " target_host
    read -p "  Puerto destino TCP (ej: 22): " target_port
    read -p "  Máx conexiones por usuario: " max_conn
    echo -e "${NC}"

    openssl req -x509 -nodes -days 3650 -newkey rsa:2048 \
        -keyout "$WS_CONFIG_DIR/server.key" \
        -out "$WS_CONFIG_DIR/server.crt" \
        -subj "/CN=ctmanager" 2>/dev/null
    ...
    cat > "$WS_CONFIG" <<EOF
{
    ...
    "enable_ws": true,
    "enable_wss": true,
    "ssl_cert": "$WS_CONFIG_DIR/server.crt",
    "ssl_key": "$WS_CONFIG_DIR/server.key",
    "max_connections_per_user": $max_conn
}
EOF

    # Descargar servidor proxy Python
    wget -q -O "$WS_SERVER" "$GITHUB_RAW/ws_server.py" 2>/dev/null || \
    cat > "$WS_SERVER" << 'PYEOF'
#!/usr/bin/env python3
import asyncio, websockets, sqlite3, ssl, json, sys, os
...
def authenticate(username, password):
    ...
    cur = conn.cursor()
    cur.execute("SELECT * FROM users WHERE username=? AND password=? AND activo=1 AND (expires_at IS NULL OR expires_at > datetime('now'))", (username, password))
    result = cur.fetchone()
    conn.close()
    return result is not None

async def proxy_data(ws, reader, writer):
    async def ws_to_tcp():
        try:
            async for msg in ws:
...
async def handler(websocket, path="/"):
    ...
    username = parts[0] if len(parts) >= 2 else None
    password = parts[1] if len(parts) >= 2 else None
    if not username or not authenticate(username, password):
        await websocket.close(1008, "Auth failed")
        return
    limit = cfg.get("max_connections_per_user", 3)
    if active_connections[username] >= limit:
        await websocket.close(1008, "Limit reached")
        return
    try:
        reader, writer = await asyncio.open_connection(cfg["target_host"], cfg["target_port"])
    except:
        await websocket.close(1011, "Connection error")
        return
    active_connections[username] += 1
    try:
        await proxy_data(websocket, reader, writer)
    finally:
        active_connections[username] = max(0, active_connections[username] - 1)

async def main():
    cfg = load_config()
    servers = []
    if cfg.get("enable_ws"):
        servers.append(await websockets.serve(handler, "0.0.0.0", cfg["ws_port"]))
    if cfg.get("enable_wss"):
        cert, key = cfg.get("ssl_cert"), cfg.get("ssl_key")
        if cert and key and os.path.exists(cert) and os.path.exists(key):
            ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
            ctx.load_cert_chain(cert, key)
            servers.append(await websockets.serve(handler, "0.0.0.0", cfg["wss_port"], ssl=ctx))
...
menu_websocket_socks() {
    while true; do
        ...
        echo -e "  ${YELLOW}7)${NC}  Ver servicios SOCKS"
        echo -e "  ${YELLOW}8)${NC}  Detener todos los SOCKS"
        echo -e "  ${RED}0)${NC}  Volver"
        echo ""
        read -p "  Opción: " op
        case $op in
