...
def leer_psiphon():
    """Estado de Psiphon y su codigo para clientes."""
    activo = os.path.exists("/etc/systemd/system/ctmanager-psiphon.service")
    if not activo:
        return {"instalado": False, "instalando": "psiphon" in TAREAS}
    est = cli("psiphon", "estado")
    cod = cli("psiphon", "codigo")
    return {
        "instalado": True,
        "activo": est.get("estado") == "active" if est.get("ok") else False,
        "codigo": cod.get("server_entry", "") if cod.get("ok") else "",
        "instalando": "psiphon" in TAREAS,
    }
...
async def datos(request: Request):
    ...
    return {"usuarios": leer_usuarios(), "sistema": estado_sistema(),
            "v2ray": leer_v2ray(), "psiphon": leer_psiphon()}
...
@app.post("/api/psiphon")
async def psiphon_accion(request: Request):
    exigir_sesion(request)
    d = await request.json()
    if d.get("accion") == "instalar":
        if "psiphon" in TAREAS:
            return {"ok": False, "error": "ya se está instalando"}
        def tarea():
            TAREAS["psiphon"] = "instalando"
            try:
                subprocess.run([CLI, "install-psiphon", "2096"],
                               capture_output=True, timeout=600)
            except Exception:
                pass
            finally:
                TAREAS.pop("psiphon", None)
        threading.Thread(target=tarea, daemon=True).start()
        return {"ok": True, "msg": "instalando"}
    return {"ok": False, "error": "acción desconocida"}
...
function pintarPsiphon(p){
  const caja = document.getElementById('psiphonCaja');
  if(!caja) return;

  if(!p || !p.instalado){
    if(p && p.instalando){
      caja.innerHTML = `<div class="info"><h4>Instalando Psiphon…</h4>
        <p>Descargando el binario oficial, puede tardar un minuto.</p></div>`;
      return;
    }
    caja.innerHTML = `<div class="caja">
        <p class="ayuda">Psiphon es otra vía de conexión, por el puerto 2096
        (HTTPS de Cloudflare). Útil donde el resto está bloqueado.</p>
        <button class="principal" onclick="instalarPsiphon()">Instalar Psiphon</button>
      </div>`;
    return;
  }
  ...
  const estado = p.activo
    ? '<span class="ok">activo</span>'
    : '<span class="mal">detenido</span>';

  caja.innerHTML = `<div class="caja">
      <div class="linea"><span>Estado</span><span class="acc">${estado}</span></div>
      <div class="linea"><span>Puerto</span><span>2096</span></div>
      <p class="ayuda" style="margin-top:10px">Este es el código que el cliente
      pega en su app (HTTP Custom, sección Psiphon) para conectarse:</p>
      <div class="enlace" id="psiCodigo">${p.codigo || '(sin código)'}</div>
      <div class="botones">
        <button onclick="copiarPsi()">Copiar código</button>
      </div>
    </div>`;
}

async function instalarPsiphon(){
  avisar('Instalando Psiphon, puede tardar un minuto.');
  await pedir('/api/psiphon', {accion:'instalar'});
  let n = 0;
  const t = setInterval(()=>{ cargar(); if(++n > 24) clearInterval(t); }, 5000);
}

async function copiarPsi(){
  const el = document.getElementById('psiCodigo');
  if(!el) return;
  ...
  try{
    ...
    try{ document.execCommand('copy'); avisar('Código copiado'); }
    catch(_){ avisar('Copialo a mano del recuadro', true); }
